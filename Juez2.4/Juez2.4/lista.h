...
template<typename Elem>
void ListLinkedSingle<Elem>::intersect(const ListLinkedSingle& other) {
    Node* preaux1;
    Node* aux1;
    Node* aux2;
    ...
    preaux1 = head;
    aux1 = this->head->next;
    aux2 = other.head->next;

    while (aux1 != nullptr) {
        //aux1 > aux2
        if ((aux2 != nullptr) && (aux1->value > aux2->value)) {
            aux2 = aux2->next;
        }
        //aux1 == aux2
        else if ((aux2 != nullptr) && (aux1->value == aux2->value)) {
            aux1 = aux1->next;
            preaux1 = preaux1->next;
            aux2 = aux2->next;
        }
        //aux1 < aux2
        else {
            aux1 = aux1->next;
            detach(preaux1, preaux1->next);
        }
    }
