...
public:
    ...
    std::pair<int, int> levelordermio() const;
...
template <typename T>
std::pair<int, int> BinTree<T>::levelordermio() const {
 
    NodePointer aux = root_node;
    int nivel = 0;
    bool asignar = false;
    std::queue<NodePointer> pending;

    //Caso base
    if (root_node == nullptr || esPrimo(root_node->elem)) {
        return { -1, 0 };
    }
    pending.push(root_node);
    //iteracion
    while (!pending.empty()) {
        ...
        if (!esPrimo(current->elem)) {

            if ((current->elem % 7) == 0) {
                return{ current->elem, nivel };
            }
