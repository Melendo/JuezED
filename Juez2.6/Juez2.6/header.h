...
template<typename Elem>
void ListLinkedSingle<Elem>::difference(ListLinkedSingle& other){
    ...
    //Si el next de other esta vacio
    while (aux2->next != nullptr) {
        //Si el next de this esta vacio
        if (aux1->next == nullptr) {
            detach(aux2);
        }
        //Si el valor de this es menor que el valor de other
        else if (aux1->next->value < aux2->next->value) {
            aux1 = aux1->next;
        }
        //Si el valor de this y de other son el mismo
        else if (aux1->next->value == aux2->next->value) {
            detach(aux1);
            aux2 = aux2->next;
        }
        //Si el valor de this es mayor que el de other
        else if (aux1->next->value > aux2->next->value) {
            detach(aux2);
        }
    }
