...
public:
    ...
    void attach(Node* ant, Node* curr)
    {
        curr->next = ant->next;
        ant->next = curr;
    }
    ...
    void repetir(const ListLinkedSingle& repet);
...
template<typename Elem>
void ListLinkedSingle<Elem>::repetir(const ListLinkedSingle& repet) {
    ...
    while (aux1->next != nullptr)
    {
        ...
        for (int i = 1; i < aux2->next->value; i++) {
            Node *curr = new Node;
            curr->value = aux1->next->value;
            attach(aux1, curr);
            aux1= aux1->next;
        }
        if (aux2->next->value == 0) {
            detach(aux1);
        }
        else if(aux1->next != nullptr)
            aux1 = aux1->next;
