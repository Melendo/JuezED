...
public:
    ...
    void colarse(QueueLinkedList<T>& colegas, int pringao);
...
template <typename T> void QueueLinkedList<T>::colarse(QueueLinkedList<T>& colegas, int pringao) {
    if (!empty()) {
        Node* aux;
        aux = this->front_node;

        while (aux != nullptr && aux->value != pringao) {
            aux = aux->next;
        }

        if (aux != nullptr && !colegas.empty()) {
            colegas.back_node->next = aux->next;
            aux->next = colegas.front_node;
