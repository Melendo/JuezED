...
public:
    ...
    void attach(Node* ant, Node* n) {
        n->next = ant->next;
        ant->next = n;
    }

    void mezclar(ListLinkedSingle<Elem>& other);
...
template<typename Elem>
void ListLinkedSingle<Elem>::mezclar(ListLinkedSingle<Elem>& other) {
    Node* aux1;
    Node* aux2;
    Node* current;
    ...
    while (aux2->next != nullptr) {
        //aux1 >= aux2 and aux1->next == nullptr
        if (aux1->next == nullptr ) {
            current = aux2->next;
            other.detach(aux2);
            this->attach(aux1, current);
            aux1 = current;
        }
