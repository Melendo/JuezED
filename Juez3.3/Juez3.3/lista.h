...
public:
    ...
    void interseccion(ListLinkedDouble<T>& list);
...
template <typename T>
void ListLinkedDouble<T>::interseccion(ListLinkedDouble<T>& list) {
    Node* aux1;
    Node* aux2;
    aux1 = this->head->next;
   aux2 = list.head->next;

    while ( aux1 != this->head ) {
        //aux1 > aux2
        if ((aux2 != list.head) && (aux1->value > aux2->value)) {
            aux2 = aux2->next;
        }
        //aux1 == aux2
        else if ((aux2 != list.head) && aux1->value == aux2->value) {
            aux1 = aux1->next;
            aux2 = aux2->next;
        }
        //aux1 < aux2
